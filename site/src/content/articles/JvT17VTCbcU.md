...
- Claude Fable 5
- Claude Code ultra
- Block Craft / Minecraftクローン
- Spotify風ワンショット
- 150万トークン
- GTA 6ラグドール物理
bulletPoints:
- time: 0
  text: 停止前のClaude Fable 5をClaude Code ultraで動かし、Minecraft・写真編集・Spotify・GTAクローンを順に1プロンプトで生成する企画。
- time: 23
  text: Minecraftは「見た目も挙動もMinecraftそっくりに作れ」と頼んだだけで自動でプロンプトを書き直し、Fable 5側がそのままビルドに突入した。
- time: 49
  text: 約1時間・150万トークン消費の末「Block Craft」が完成し、ブロック破壊や無限資源など主要メカニクスが動作した。
- time: 814
  text: 次に試したPhotoshop風画像編集は約6分で完成、プロンプトに書いた機能は全て載るが、書かないと出ないという素直な挙動だった。
- time: 825
  text: Spotify風アプリはサイドバー・アルバムアートグリッド・再生コントロール・オーディオビジュアライザーまで含めて11分でワンショット生成された。
- time: 884
  text: デモトラックも自動で書き起こされ「いいね」「検索」「ライブラリ」まで含めた完成度で、UIだけでなくアプリ全体として動作していた。
- time: 1693
  text: GTA 6級は流石にワンショットでは無理だが、ラグドール物理や爆発時の車挙動は本物に肉薄しており、繰り返せば本物に届く印象だと結論づけた。
sections:
- heading: Minecraft — 1時間と150万トークンで動くクローンに
  time: 0
  body: 'Prompt PotatoはClaude Fable 5(公開停止直前)に対し、Claude Codeの「ultra」モードで複雑なクローン生成を順に試している。最初の題材はMinecraft。「見た目も挙動もMinecraftそっくりに作れ」と入力したところ、モデル側で勝手にプロンプトを練り直してそのままビルドに突入。
...
- [00:00] 停止前のClaude Fable 5をClaude Code ultraで動かし、Minecraft・写真編集・Spotify・GTAクローンを順に1プロンプトで生成する企画。
- [00:23] Minecraftは「見た目も挙動もMinecraftそっくりに作れ」と頼んだだけで自動でプロンプトを書き直し、Fable 5側がそのままビルドに突入した。
- [00:49] 約1時間・150万トークン消費の末「Block Craft」が完成し、ブロック破壊や無限資源など主要メカニクスが動作した。
- [13:34] 次に試したPhotoshop風画像編集は約6分で完成、プロンプトに書いた機能は全て載るが、書かないと出ないという素直な挙動だった。
- [13:45] Spotify風アプリはサイドバー・アルバムアートグリッド・再生コントロール・オーディオビジュアライザーまで含めて11分でワンショット生成された。
- [14:44] デモトラックも自動で書き起こされ「いいね」「検索」「ライブラリ」まで含めた完成度で、UIだけでなくアプリ全体として動作していた。
- [28:13] GTA 6級は流石にワンショットでは無理だが、ラグドール物理や爆発時の車挙動は本物に肉薄しており、繰り返せば本物に届く印象だと結論づけた。

## セクション

### Minecraft — 1時間と150万トークンで動くクローンに

- 時刻: 00:00

Prompt PotatoはClaude Fable 5(公開停止直前)に対し、Claude Codeの「ultra」モードで複雑なクローン生成を順に試している。最初の題材はMinecraft。「見た目も挙動もMinecraftそっくりに作れ」と入力したところ、モデル側で勝手にプロンプトを練り直してそのままビルドに突入。

約1時間で『Block Craft』が完成し、ブロック破壊・無限資源・クラフト風UIまでひと通り動作。消費トークンは約150万で、コストはかかるものの「本物のMinecraftを起動したのかと一瞬騙された」と評する仕上がりに到達している。

### Photoshop / Spotify風アプリは数分〜10分でワンショット

- 時刻: 13:34

より定型的なUIに関しては、生成時間は一気に短くなる。Photoshop風の画像編集は約6分、Spotify風アプリは約11分でワンショット生成された。Spotify側はダークUIにサイドバー、アルバムグリッド、検索バー、再生・一時停止・スキップ・シャッフル・進捗バー・ボリューム、そしてオーディオビジュアライザーまで盛り込んだ仕様だった。

面白いのは、Fable 5がデモ用に楽曲タイトルや「Neon Skyline」「Glass Bloom」などの曲名まで自前で作ってきた点だ。曲数は限定的でも、検索・ライク・ライブラリ表示も実装されており、UIだけでなくアプリとして体験できる完成度に達していた。

### GTA 6 — ワンショットでは無理、でも『ラグドール物理』は本物級
